...
class MikrotikCoordinator(DataUpdateCoordinator[None]):
    ...
    def get_access(self) -> None:
        ...
        user = tmp_user.get(username)
        if isinstance(user, dict):
            group = user.get("group")
            group_entry = tmp_group.get(group) if group else None
            if isinstance(group_entry, dict):
                policy = group_entry.get("policy")
    ...
    def get_filter(self) -> None:
        ...
        self.ds["filter"] = parse_api(
            ...
            vals=[
                ...
                {"name": "connection-nat-state", "default": "any"},
                {"name": "connection-mark", "default": "any"},
                {"name": "packet-mark", "default": "any"},
